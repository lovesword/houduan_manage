...
class Corder extends Api
{
    ...
    private function setPayParam($order){
        ...
        if($openid=='o3W0Y4_2rFmIi00R71ClYr1UpCyU'||$openid=='o3W0Y430RAOkltn2mJVqaUK9w6q8'){
           // $total_fee=1;
        }
        if($total_fee<=0){
           error_json('金额有误');
...
//        订单保存验证
        ...
        $wxinfo=$wx->pay($user['openid'],$order_model->pay_amount,$order_model->id,'Order','商城订单'.$order_model->order_no);
...
//    获取订单详情
    ...
    public function payOrder(){
        ...
        foreach ($ordergoodses as $ordergoods) {
            ...
            $goods->checkState($ordergoods->goods_id);
        }

        $wx=new Cwx();
        $user = User::get($order->user_id);
        $wxinfo=$wx->pay($user['openid'],$order->pay_amount,$order->id);
    ...
    public function getOrderSet(){
        ...
        $orders = Order::where('t1.state not in (1,6)')
            ->alias('t1')
...
//            $val['endphone']=substr($val['phone'],-4);
//        }
