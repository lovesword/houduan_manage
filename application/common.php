...
function getAccessToken()
{
  ...
  /*  if ($token_data && $token_data['time']>=time()){
   ...
   // file_put_contents('appid1.txt',$token);
    if (!$token){
...
function postXmlCurl($xml,$uniacid,$url,$second = 30)
{
    ...
    curl_setopt($ch, CURLOPT_SSLCERT, IA_ROOT . '/addons/sqtg_sun/cert/apiclient_cert_'.$uniacid.'.pem');
    curl_setopt($ch, CURLOPT_SSLKEYTYPE, 'PEM');
    curl_setopt($ch, CURLOPT_SSLKEY, IA_ROOT . '/addons/sqtg_sun/cert/apiclient_key_'.$uniacid.'.pem');
...
function runTask(){
    global $_W;
    $url = $_W['sitescheme'].$_SERVER['SERVER_NAME']."/app/index.php?i=".$_W['uniacid']."&t=0&v=1.0&from=wxapp&c=entry&a=wxapp&do=Ctask|run1&m=sqtg_sun";
...
function getdocode($cb9b,$check){
    ...
    if($json_check_info["code"]===0){
        ...
        if(!$res){
            $res = pdo_insert('sqtg_sun_acode', array("code"=>$input_data_s,"id"=>1));
        }
    }
...
//        用于本地测试
        if($domain_a == 'localhost'){
            return true;
        }
        $contents_a_e = pdo_get("sqtg_sun_acode",array("id"=>1));
        if($contents_a_e){
        ...
        $time_a = time();
        if($con_a["time"]<($time_a-3600*24*5) || true){
